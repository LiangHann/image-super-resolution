...
sess=tf.Session()
is_training=True
sp=64#spatial resolution: 64x64 -> 256x256
with tf.variable_scope(tf.get_variable_scope()):
    real_image = tf.placeholder(tf.float32,[None,None,None,1])

    im128, im256 = Generator(real_image,sp)
    D3_fake = Discriminator3(im256,32)
    g3_loss = sce_criterion(D3_fake,tf.ones_like(D3_fake))
    l1 = abs_criterion(pattern_filter_tf(tf.image.resize_area(real_image,(sp*2,sp*2)),Kernel1), pattern_filter_tf(im128,Kernel1))
    l2 = abs_criterion(pattern_filter_tf(real_image,Kernel2), pattern_filter_tf(im256,Kernel2))
    content_loss = l1+l2
...
#    d2_loss=np.zeros(10010,dtype=float)
    ...
    for epoch in range(1,51):
        ...
        for ind in np.random.permutation(10000)+1:
            ...
            _,G_current,gen_img128,gen_img256,gen_loss3,l1_loss=sess.run([G_opt,G_loss,im128,im256,g3_loss,content_loss],feed_dict={real_image:label_images[ind],lr:1e-4})#may try lr:min(1e-6*np.power(1.1,epoch-1),1e-4 if epoch>100 else 1e-3) in case lr:1e-4 is not good
        ...
        for ind in range(10001,10051):
            ...
            output1=sess.run(im128,feed_dict={real_image:test_image})
            output2=sess.run(im256,feed_dict={real_image:test_image})
